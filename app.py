...
def init_web_db():
  ...
  initial_products = [
      ('Pelmen /300 gr', 1000, 6200, 14000.0),
      ('Pelmen /500 gr', 1000, 9900, 24000.0),
      ('Pelmen rasepnoy /kg', 1000, 19500, 46000.0),
      ('Teftel /300 gr', 1000, 12000, 23000.0),
      ('Pelmen ossarti /500 gr', 1000, 18000, 30000.0),
      ('Pelmen ossarti /300 gr', 1000, 11500, 20000.0),
      ('Golubtsi /500 gr', 1000, 14000, 25000.0),
      ('Tok doʻlma /300 gr', 1000, 11000.0, 25000.0),
      ('Karam doʻlma /300 gr', 1000, 11000.0, 25000.0),
      ('Somsa kesilgan /800 gr', 1000, 6000.0, 17000.0),
      ('Oʻrama xamir', 1000, 6000.0, 17000.0),
      ('KFC', 1000, 27500 30000.0),
      ('KFC Gulim', 1000.0, 15000, 30000.0),
      ('Osh masalliq 500 gr', 1000, 6000.0, 13000.0),
      ('Osh masalliq 1 kg', 1000, 7000.0, 15000.0),
      ('Lagʻmon', 10000, 2000.0, 6000.0),
      ('Kotlet', 1000, 11000.0, 25000.0),
      ('Lavash hamiri', 1000, 3800.0, 7000.0),
  ]
...
def operator_dashboard():
  ...
  chart_labels = list(cat_stats.keys()) if cat_stats else ["Ma'lumot yo'q"]
  chart_data = list(cat_stats.values()) if cat_stats else [1]
  chart_colors = (
  ...
  cat_table_data = [
      {
          ...
          'amount': v,
          'percent': (
              round((v / daily_sum * 100), 1) if daily_sum > 0 else 0
          ),
          'color': palette[i % len(palette)],
      }
      for i, (k, v) in enumerate(cat_stats.items())
  ]
